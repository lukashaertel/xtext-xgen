...
public class Iteration
{
	...
	public Index<Node> iterate(EObject element)
	{
		return new GrammarSwitch<Index<Node>>()
		{
			...
			{
				return iterate(object.getRhs()).mapPresent(x -> new Container(object, x));
			}
			...
			public Index<Node> caseMultiplicity(Multiplicity object)
			{
				...
				return iterate(object.getOperand()).combinations(min, max, INNER_PER_OUTER).mapPresent(x -> Container.fromIterable(object, x));
			...
			{
				return Index.late(() -> iterate(object.getTarget()).mapPresent(x -> new Container(object, x)));
			}

			@Override
			public Index<Node> caseSequence(Sequence object)
			{
				Index<Iterable<Node>> r = iterate(object.getOperands().get(0)).mapPresent(i -> Collections.singleton(i));

				for (int i = 1; i < object.getOperands().size(); i++)
					r = r.productPresent(iterate(object.getOperands().get(i)), (x, y) -> Iterables.concat(x, Collections.singleton(y)));

				return r.mapPresent(x -> Container.fromIterable(object, x));

				// Index<Node> l = iterate(object.getLeft());
				// Index<Node> r = iterate(object.getRight());
				//
				// return l.productPresent(r, (x, y) -> container(object, x,
				// y));
			}
	...
	public static String[] candidateStates = { "status1", "andererStatus", "kleingeld" };

	public static Node[] replaceInput(Node[] ns)
	{
		Node[] r = new Node[ns.length];

		for (int i = 0; i < ns.length; i++)
			if (ns[i] instanceof Leaf)
			{
				Leaf l = (Leaf) ns[i];

				if ("<<input>>".equals(l.value))
					r[i] = new Leaf(candidateInputs[Math.max(0, Float.floatToIntBits((float) Math.random())) % candidateStates.length]);
				else
					r[i] = ns[i];
			}
			else
	...
	public static Node[] replaceState(Node[] ns)
	{
		Node[] r = new Node[ns.length];
		...
		for (int i = 0; i < ns.length; i++)
			if (ns[i] instanceof Leaf)
			{
				...
				if ("<<state>>".equals(l.value))
					r[i] = new Leaf(candidateStates[Math.max(0, Float.floatToIntBits((float) Math.random())) % candidateStates.length]);
				else
					r[i] = ns[i];
			}
			else
	...
	{
		Definition transition = definition("transition", sequence(Arrays.asList(keyword("<<input>>"), keyword("->"), keyword("<<state>>"), keyword(";"))));
		Definition state = definition("state",
				sequence(Arrays.asList(keyword("<<one of initial>>"), keyword("state"), keyword("<<state>>"), keyword("{"), multiplicity(reference(transition), 2, true, 4), keyword("}"))));
		Definition model = definition("model", multiplicity(reference(state), 1, true, 3));
		...
		for (long l = start; l < start + amount; l++)
		{
			...
			if (n instanceof Container)
			{
				...
				System.out.println(c.transformAllChildren(Iteration::replaceState).transformAllChildren(Iteration::replaceInput));
			}
			else
			{
